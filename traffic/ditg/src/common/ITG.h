...
inline void getInfo(struct addrinfo *Host, int &tmpPort, char *HelpAddress) {
    ...
    getnameinfo(Host->ai_addr, Host->ai_addrlen, HelpAddress, INET6_ADDRSTRLEN, NULL, 0,
                NI_NUMERICHOST);
}


inline int writeInBufferStandard(info *infos, int FlowId, int ptrSeqNum, char *SrcHost,
                                 char *DestHost, in_port_t tmp_portSrc, in_port_t tmp_portDest,
    ...
    PRINTD(2, "TxTime : %02u:%02u:%06lf ", (unsigned int) infos->txTime1,
           (unsigned int) infos->txTime2, infos->txTime3);
    PRINTD(2, "RxTime : %02u:%02u:%06lf ", (unsigned int) infos->rxTime1,
           (unsigned int) infos->rxTime2, infos->rxTime3);
    PRINTD(2, "Size : %5u\n\n", infos->size);
    return 0;
}
...
inline int writeInBufferShort(info *infos, int FlowId, int ptrSeqNum, char *SrcHost,
    ...
    PRINTD(2, "\tSrc : %16s/%hu  ", infos->srcAddr, infos->srcPort);
    PRINTD(2, "Dest : %16s/%hu  ", infos->destAddr, infos->destPort);
    PRINTD(2, "TxTime : %u:%u:%lf ", (unsigned int) infos->txTime1, (unsigned int) infos->txTime2,
           infos->txTime3);
    PRINTD(2, "RxTime : %u:%u:%lf ", (unsigned int) infos->rxTime1, (unsigned int) infos->rxTime2,
           infos->rxTime3);
    PRINTD(2, "Size : %5u\n\n", infos->size);
    return 0;
}
...
inline void infosHostToNet(struct info *infos) {
    ...
    infos->rxTime1 = htonl(
            infos->rxTime1 * 3600 + infos->rxTime2 * 60 + (unsigned int) floor(infos->rxTime3));
    infos->rxTime2 = htonl(
            (uint32_t) ((infos->rxTime3 - (unsigned int) floor(infos->rxTime3)) * 1000000));
    infos->rxTime3 = 0;
    ...
    infos->txTime1 = htonl(
            infos->txTime1 * 3600 + infos->txTime2 * 60 + (unsigned int) floor(infos->txTime3));
    infos->txTime2 = htonl(
            (uint32_t) ((infos->txTime3 - (unsigned int) floor(infos->txTime3)) * 1000000));
    infos->txTime3 = 0;
...
inline int
TCPrecvPacket(unsigned char *payload, int newSock, int preambleSize, int payloadLogType) {
