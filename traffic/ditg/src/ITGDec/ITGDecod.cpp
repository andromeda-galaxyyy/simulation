...
ofstream *FlowPresent(unsigned int flowId, char *destAddr, char *srcAddr, unsigned int srcPort,
                      unsigned int destPort);

struct info *readline(ifstream *fbin, FILE *ftxt, int filetype);

struct info *found(unsigned int flowId, char *srcAddr, char *destAddr, unsigned int srcPort,
                   unsigned int destPort, unsigned int seqNum);
...
int main(int argc, char *argv[]) {
                    ...
                    case 'c':
                        if ((argc < 2) || (argv[1][0] == '-')) {
                            ...
                            if (mscombinedstats <= 0) {
                                ...
                                flagdelay = 1;
                                msdelay = mscombinedstats;
                                flagbitrate = 1;
                                msbitrate = mscombinedstats;
                                flagjitter = 1;
                                msjitter = mscombinedstats;
                                flagpktloss = 1;
                                mspktloss = mscombinedstats;
    ...
    if (!((flagdelay) || (flagjitter) || (flagps) || (flagidt) || (flagbitrate) || (elab_type) ||
          (flaglogout) || (flagpktloss)
    ...
    while (size > 0) {
        ...
        if (size > 0) {
            mark();
            if (flagreconstruct) {
                struct info *send_info = found((*infos).flowId, (*infos).srcAddr, (*infos).destAddr,
                                               (*infos).srcPort,
            ...
            if (flagoctave) {
                ...
                fprintf(octavefpout, "%u %u %lf ", (*infos).txTime1, (*infos).txTime2,
                        (*infos).txTime3);
                fprintf(octavefpout, "%u %u %lf ", (*infos).rxTime1, (*infos).rxTime2,
                        (*infos).rxTime3);
            ...
            if (errornum > 0) {
                ...
                if (flagbitrate || flagjitter || flagdelay || flagpktloss || flagsplit)
                    split();
                sectxTime = ((double) (*infos).txTime1 * 3600 + (double) (*infos).txTime2 * 60 +
                             (*infos).txTime3);
                ...
                delvariation = delvariation + ((deltatime(sectxTime, secrxTime)) *
                                               (deltatime(sectxTime, secrxTime)));
    ...
    if (elab_type == 1) {
        ...
        for (z = 0; z < flownum; z++) {
            ...
            flowinfo[z].delvariation =
                    flowinfo[z].delvariation - (flowinfo[z].avdelay * flowinfo[z].avdelay);
        ...
        if (pktrecv > 0 && pktloss > 0) {
            printf("Packets dropped          = %13.0u (%3.2lf %%)\n", pktloss,
                   (double) (100 * pktloss / (pktrecv + pktloss)));
...
ofstream *FlowPresent(unsigned int flowId, char *destAddr, char *srcAddr, unsigned int srcPort,
    ...
    while (i < MaxNumFlow) {
        if ((memFile[i].flowId == flowId) && (strcmp(memFile[i].srcAddr, srcAddr) == 0) &&
            (strcmp(memFile[i].destAddr, destAddr)
...
struct info *found(unsigned int flowId, char *srcAddr, char *destAddr, unsigned int srcPort,
    ...
    while (sendlog.good() && (!flagfound)) {
        ...
        if (((*infos_loc).flowId == flowId) && (strcmp((*infos_loc).srcAddr, srcAddr) == 0) &&
            (strcmp((*infos_loc).destAddr,
                    destAddr) == 0) && ((*infos_loc).srcPort == srcPort) &&
            ((*infos_loc).destPort == destPort) && ((*infos_loc).seqNum
                                                    == seqNum))
            flagfound = 1;
    }
    if (flagfound) {
...
struct info *readline(ifstream *fbin, FILE *ftxt, int filetype) {
    ...
    if (filetype == 0) {
        ...
        if (fscanf(ftxt, "Flow>%04u ", &(*infos_loc).flowId) != EOF) {
            ...
            fscanf(ftxt, "Dest>%1c%45c/%u ", &buffer, (*infos_loc).destAddr,
                   &(*infos_loc).destPort);
            fscanf(ftxt, "txTime>%u:%u:%lf ", &(*infos_loc).txTime1, &(*infos_loc).txTime2,
                   &(*infos_loc).txTime3);
            fscanf(ftxt, "rxTime>%u:%u:%lf ", &(*infos_loc).rxTime1, &(*infos_loc).rxTime2,
                   &(*infos_loc).rxTime3);
#endif
#endif
            fscanf(ftxt, "Size>%5u\n", &(*infos_loc).size);
...
void setline(double txTime, double rxTime) {
    ...
    if (flownum == 0) {
        ...
        PRINTD(2, "Flow=%d Seq=%u pktrecv=%lu pktloss=%lu pktdup=%u\n",
               infos->flowId, infos->seqNum, flowinfo[0].pktrecv, flowinfo[0].pktloss,
               flowinfo[0].pktdup);
    } else {
        int found = 0;
        int index = 0;
        if (flagfilter == 2) {
            ...
            for (unsigned int z = 0; z < flownum; z++)
                if ((flowinfo[z].flowId == (*infos).flowId) &&
                    (strcmp(flowinfo[z].srcaddress, (*infos).srcAddr) == 0)
                    && (strcmp(flowinfo[z].destaddress, (*infos).destAddr) == 0) &&
                    (flowinfo[z].srcport
        ...
        if (found) {
            ...
            flowinfo[index].delvariation =
                    flowinfo[index].delvariation + (currentdelay * currentdelay);
            ...
            if ((flowinfo[index].maxSeq - flowinfo[index].minSeq + 1) <
                (flowinfo[index].pktrecv - flowinfo[index].pktdup)) {
                flowinfo[index].pktdup =
                        -flowinfo[index].maxSeq + flowinfo[index].minSeq + flowinfo[index].pktrecv -
                        1;
                flowinfo[index].pktloss = 0;
            } else {

                if ((flowinfo[index].maxSeq - flowinfo[index].minSeq - flowinfo[index].pktrecv +
            ...
            flownum++;
            flowinfo = (struct stflowinfo *) realloc(flowinfo,
                                                     (flownum + 1) * sizeof(struct stflowinfo));
            ...
            PRINTD(2, "Flow=%d Seq=%d pktrecv=%ld pktloss=%ld pktdup=%d lossEvents=%d\n",
                   ...
                   flowinfo[flownum - 1].pktloss, flowinfo[flownum - 1].pktdup,
                   flowinfo[flownum - 1].lossEvents);
        }
    }
}
...
void split() {
    ...
    IdFile = FlowPresent((*infos).flowId, (*infos).destAddr, (*infos).srcAddr, (*infos).srcPort,
                         (*infos).destPort);
    if (IdFile != NULL) {
                ...
                if (size == 0) {
                    if (flagbit)
                        fprintf(outfilebit, "%lf %lf\n", intclosedbit * (double) msbitrate / 1000,
                                ...
                                / msbitrate);
                    if (flagjit)
                        fprintf(outfilejit, "%lf %lf\n", intclosedjit * (double) msjitter / 1000,
                                tempjitter
                                / (double) (pktjitter - 1));
                    if (flagdel)
                        fprintf(outfiledel, "%lf %lf\n", intcloseddel * (double) msdelay / 1000,
                                tempdelay
                                / (double) pktdelay);
                    if (flagpkt)
                        fprintf(outfilepkt, "%lf %lu\n", intclosedpkt * (double) mspktloss / 1000,
                                temppktloss);
                    break;
                } else {
                    pktrecv++;
                    ...
                    currentdelay = deltatime(
                            ((double) (*infos).txTime1 * 3600) + ((double) (*infos).txTime2 * 60)
                    ...
                    if (flagbit) {
                        if ((secrxTime - secinitbitrate) > (msbitrate / 1000)) {
                            ...
                            double temporary =
                                    deltatime(secfirstline_gl, secrxTime) * 1000 / msbitrate;
                            unsigned long int inttoclose = (unsigned long int) (ceil(temporary) -
                                                                                intclosedbit
                                                                                - 1);
                            if (inttoclose > 0) {
                    ...
                    if (flagjit) {
                        if ((secrxTime - secinitjitter) > (msjitter / 1000)) {
                            ...
                            double temporary =
                                    deltatime(secfirstline_gl, secrxTime) * 1000 / msjitter;
                            unsigned long int inttoclose = (unsigned long int) (ceil(temporary) -
                                                                                intclosedjit
                                                                                - 1);
                            if (inttoclose > 0) {
                    ...
                    if (flagdel) {
                        if ((secrxTime - secinitdelay) > (msdelay / 1000)) {
                            ...
                            double temporary =
                                    deltatime(secfirstline_gl, secrxTime) * 1000 / msdelay;
                            unsigned long int inttoclose = (unsigned long int) (ceil(temporary) -
                                                                                intcloseddel
                                                                                - 1);
                            if (inttoclose > 0) {
                    ...
                    if (flagpkt) {
                        if ((secrxTime - secinitpktloss) > (mspktloss / 1000)) {
                            ...
                            double temporary =
                                    deltatime(secfirstline_gl, secrxTime) * 1000 / mspktloss;
                            unsigned long int inttoclose = (unsigned long int) (ceil(temporary) -
                                                                                intclosedpkt
                                                                                - 1);
                            if (inttoclose > 0) {
...
void mergeCombined() {
    ...
    while (1) {
        ...
        while (memFile[i].flowId != (unsigned int) -1) {
            ...
            if ((memFile[i].flowId == (unsigned int) -1) && (flagbreak != flownum)) {
                fprintf(fpout, " %lf %lf %lf %lf\n", valuesum_bit, valuesum_del, valuesum_jit,
                        valuesum_pkt);
            }
        }
        if (flagbreak == flownum)
