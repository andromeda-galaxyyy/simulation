...
int modeManager(int argc, char *argv[]) {
    ...
    do {
        char *endline;
        size_r = recvfrom(managerSock, buffer, MAX_FLOW_LINE_SIZE - 3, 0,
                          (struct sockaddr *) &ManagerIP, &ManagerIPslen);
        ...
        size_s = sendto(managerSock, buffer, size_r + 3, 0, (struct sockaddr *) &ManagerIP,
                        ManagerIPslen);
        if (size_s < 0) {
...
int main(int argc, char *argv[]) {
    ...
    PRINTD(1, "main: SignalingTime Time: %lu.%06lu sec\n", (long unsigned int) RifTime.tv_sec,
           (long unsigned int) RifTime.tv_usec);
        ...
        if (argv[0][0] == '-') {
            switch (argv[0][1]) {
                    ...
                    if ((argc < 2) || (argv[1][0] == '-') ||
                        ...
                        if (findL4Proto(argv[1]) == LX_ERROR_BYTE) {
                            ...
                            if (getaddrinfo(argv[1], NULL, &hint, &logHost))
                                ReportErrorAndExit("General parser",
                                                   " Invalid log-server address or protocol",
                                                   programName, 0);
                            argc -= 1;
                            argv += 1;
                        } else {
                        ...
                        if ((argc >= 2) && (argv[1][0] != '-') &&
                            ...
                            if (findL4Proto(argv[1]) == LX_ERROR_BYTE) {
                                ...
                                if (getaddrinfo(argv[1], NULL, &hint, &logHost))
                                    ReportErrorAndExit("General parser",
                                                       " Invalid log-server address", programName,
                                                       0);
                                argc -= 2;
                                argv += 2;
                            } else {
                ...
                case 'x':
                    ...
                    break;
                default:
                    char tempS[sizeof("Option ") + sizeof(argv[0][1]) +
                               sizeof(" can not be used as a global option in the multy flow mode !")];
                    strcpy(tempS, "Option ");
                    strcat(tempS, argv[0]);
...
void *flowParser(void *param) {
        ...
        if (argv[h][0] == '-') {
            ...
            switch (argv[h][1]) {
                case 'p':
                    if (argv[h][2] != 'o') {
                        ...
                        ReportErrorAndExit("General parser",
                                           "Error in parsing \"p\" or \"-poll\" options",
                                           programName, id);
                    }
                    break;
                case 'm':
                    ...
                    if (flows[id].meter == 255) {
                        ReportErrorAndExit("General parser", "Invalid measure type", programName,
                                           0);
                    }
                    if ((flows[id].meter == METER_RTTM) &&
                        (flows[id].minPayloadSize < StandardMinPayloadSize)) {
                ...
                case 'a': {
                    ...
                    if ((argc < 2) || getaddrinfo(argv[h + 1], NULL, &hint, &flows[id].DestHost))
                        ReportErrorAndExit("General parser", "Invalid destination address",
                                           programName, id);
#ifndef NOIPV6
                    if (flows[id].DestHost->ai_family == PF_INET6
                        && IN6_IS_ADDR_LINKLOCAL(
                                &((struct sockaddr_in6 *) (flows[id].DestHost->ai_addr))->sin6_addr)
                        && ((struct sockaddr_in6 *) (flows[id].DestHost->ai_addr))->sin6_scope_id ==
                           0) {
                    ...
                    if (SigDestHostSpecify == false) {
                        ...
                        if ((argc < 2) ||
                            getaddrinfo(argv[h + 1], NULL, &hint, &flows[id].SigDestHost))
                            ReportErrorAndExit("General parser",
                        ...
                        if (flows[id].SigDestHost->ai_family == PF_INET6
                            && IN6_IS_ADDR_LINKLOCAL(
                                    &((struct sockaddr_in6 *) (flows[id].SigDestHost->ai_addr))->sin6_addr)
                            &&
                            ((struct sockaddr_in6 *) (flows[id].SigDestHost->ai_addr))->sin6_scope_id ==
                            0) {
                ...
                case 'i': {
                    if (flows[id].srcAddrSpecify == false) {
                        if (argc < 2)
                            ReportErrorAndExit("General parser", "Invalid interface", programName,
                                               id);
                        ...
                        ReportErrorAndExit("General parser",
                                           "use -i <interface> OR -sa <source_address>",
                                           programName, id);
                    break;
                }
#endif
                case 'r':
                    if (argv[h][2] == 'p') {
                        if (argc < 2)
                            ReportErrorAndExit("General parser", "Invalid port number", programName,
                                               id);
                        ...
                        if (argc < 2)
                            ReportErrorAndExit("General parser", "Invalid Serial Port", programName,
                                               id);
                        ...
                        ReportErrorAndExit("General parser",
                                           strcat(strcat(strcpy(tempS, "What is "), argv[h]), " ?"),
                                           programName, id);
                    }
                    break;
                case 'f':
                ...
                case 'T':
                    if (argc < 2)
                        ReportErrorAndExit("General parser", "Invalid Protocol Type", programName,
                                           id);
                ...
                case 's':
                    if (strlen(argv[h]) == 2) {
                        if ((argc < 2) || ((seed = strtod(argv[h + 1], NULL)) >= 1.0) ||
                            (seed <= 0.0))
                            ReportErrorAndExit("General parser", "Invalid seed (0 < seed < 1)",
                                               programName, id);
                        h += 2;
                        argc -= 2;
                    } else if (argv[h][2] == 'p') {
                        ...
                        if (flows[id].iface == NULL) {
                            ...
                            if ((argc < 2) ||
                                getaddrinfo(argv[h + 1], NULL, &hint, &flows[id].SrcHost))
                                ReportErrorAndExit("General parser", "Invalid source address",
                                                   programName, id);
#ifndef NOIPV6
                            if (flows[id].SrcHost->ai_family == PF_INET6
                                && IN6_IS_ADDR_LINKLOCAL(
                                        &((struct sockaddr_in6 *) (flows[id].SrcHost->ai_addr))->sin6_addr)
                                &&
                                ((struct sockaddr_in6 *) (flows[id].SrcHost->ai_addr))->sin6_scope_id ==
                                0) {
                                printf("** WARNING **: IPv6 address scope id not specified by the sender\n");
                            }
#endif
                ...
                case 'C':
                    ...
                    ConstantRV = new Constant(1);
                    flows[id].IntArriv = new SumRandom(
                            1000.0 / strtod(argv[h + 1], NULL) * (*ConstantRV));
                    h += 2;
                    argc -= 2;
                    break;
                case 'U':
                ...
                case 'W':
                    if ((argc < 3) || (strtod(argv[h + 1], NULL) <= 0) ||
                        (strtod(argv[h + 2], NULL) <= 0))
                        ReportErrorAndExit("Protocol Parser",
                                           "Invalid Weibull Distribution parameter values",
                                           programName, id);
                    a = (Real) strtod(argv[h + 1], NULL);
                    b = (Real) strtod(argv[h + 2], NULL);
                    delete flows[id].IntArriv;
                ...
                case 'w':
                    if ((argc < 3) || (strtod(argv[h + 1], NULL) <= 0) ||
                        (strtod(argv[h + 2], NULL) <= 0))
                        ReportErrorAndExit("Protocol Parser",
                                           "Invalid Weilbull Distribution Parameter values",
                                           programName, id);
                    a = (Real) strtod(argv[h + 1], NULL);
                    b = (Real) strtod(argv[h + 2], NULL);
                    WeibullRV = new Weibull(a, b);
                ...
                case 'F':
                    ...
                    if (argv[h][2] == 'p') {
                        ...
                        if ((argc < 2) || (argv[h + 1][0] == '-'))
                            ReportErrorAndExit("General parser",
                                               "Invalid name file specified for -Fp option",
                                               programName, id);
                        ...
                        if (flows[id].dimPayloadBuffer == 0) {
                            ...
                            if (ptrFile == NULL)
                                ReportErrorAndExit("flowParser",
                                                   "Error in opening the file of payload",
                                                   programName, id);
                            ...
                            if (flows[id].ptrPayloadBuffer == NULL)
                                ReportErrorAndExit("flowParser",
                                                   "Non enough memory for load the file",
                                                   programName, id);

                            fseek(ptrFile, 0, SEEK_SET);
                            bytesRead = fread(flows[id].ptrPayloadBuffer, 1,
                                              flows[id].dimPayloadBuffer, ptrFile);

                            if ((bytesRead != flows[id].dimPayloadBuffer) || ferror(ptrFile))
                                ReportErrorAndExit("flowParser",
                                                   "Error in reading the file of payload",
                                                   programName, id);
                            else
                        ...
                        if ((argc < 2) || (argv[h + 1][0] == '-'))
                            ReportErrorAndExit("General parser",
                                               "Invalid name file specified for -Fs option",
                                               programName, id);
                        ...
                        if (ptrFile == NULL)
                            ReportErrorAndExit("flowParser",
                                               "Error in opening the file of the packets sizes",
                                               programName, id);
                        ...
                        if (fgets(line, sizeof(line) - 1, ptrFile) == NULL) {
                            ReportErrorAndExit("flowParser",
                                               "Error in reading the file of the packets sizes",
                                               programName, id);
                        }
                        lineNumber = 1;
                            ...
                            if (strcmp(line, "\n") != 0) {
                                if ((atoi(line) <= 0) || (atoi(line) > DIM_MAX_IP_PACKET)) {
                                    printf("\nError in reading the file of the packets sizes: negative, too big or invalid number at line %d\n",
                                           lineNumber);
                                    ReportErrorAndExit("flowParser",
                                                       "Error in reading the file of the packets sizes: negative, too big or invalid number",
                                                       programName, id);
                                }
                                tmpInt = atoi(line);
                                PRINTD(4, "flowParser: Size read = %d ; value n° = %d\n", tmpInt,
                                       (int) flows[id].vectSize.size() + 1);
                        ...
                        if ((argc < 2) || (argv[h + 1][0] == '-'))
                            ReportErrorAndExit("General parser",
                                               "Invalid name file specified for -Ft option",
                                               programName, id);
                        ...
                        if (ptrFile == NULL)
                            ReportErrorAndExit("flowParser", "Error in opening the file of the IDT",
                                               programName, id);
                        ...
                        if (fgets(line, sizeof(line) - 1, ptrFile) == NULL) {
                            ReportErrorAndExit("flowParser", "Error in reading the file of the IDT",
                                               programName, id);
                        }
                        lineNumber = 1;
                            ...
                            if (strcmp(line, "\n") != 0) {
                                if ((strtod(line, NULL) < 0) || (strtod(line, NULL) == HUGE_VAL)) {
                                    printf("\nError in reading the file of the IDT: negative, too big or invalid number at line %d\n",
                                           lineNumber);
                                    ReportErrorAndExit("flowParser",
                                                       "Error in reading the file of the IDT: negative, too big or invalid number",
                                                       programName, id);
                                }
                                tmpFloat = strtod(line, NULL);
                                PRINTD(2, "flowParser:  Time read = %f ; value n° = %d\n", tmpFloat,
                                       (int) flows[id].vectTime.size() + 1);
                ...
                case 'S':
                    if (argv[h][2] == 's') {
                        if (argv[h][3] == 'a') {
                            ...
                            if ((argc < 2) ||
                                getaddrinfo(argv[h + 1], NULL, &hint, &flows[id].SigSrcHost))
                                ReportErrorAndExit("General parser",
                                                   "Invalid source address for the signal channel",
                                                   programName,
                                                   id);
                            ...
                            if (flows[id].SigSrcHost->ai_family == PF_INET6
                                && IN6_IS_ADDR_LINKLOCAL(
                                        &((struct sockaddr_in6 *) (flows[id].SigSrcHost->ai_addr))->sin6_addr)
                                &&
            ...
            switch (flows[id].l7Proto) {
                case L7_PROTO_TELNET:
                    ...
                    telnetParser(&flows[id].IntArriv, &flows[id].PktSize, flows[id].IntArrivDistro,
                                 flows[id].PktSizeDistro);
                ...
                case L7_PROTO_DNS:
                    ...
                    dnsParser(&flows[id].IntArriv, &flows[id].PktSize, flows[id].IntArrivDistro,
                              flows[id].PktSizeDistro);
                    h++;
                    argc--;
                    break;
                ...
                case L7_PROTO_CSATTIVA:
                    CSParsera(&flows[id].IntArriv, &flows[id].PktSize, flows[id].IntArrivDistro,
                              flows[id].PktSizeDistro);
                    flows[id].l4Proto = L4_PROTO_UDP;
                    h++;
                    argc--;
                ...
                case L7_PROTO_CSINATTIVA:
                    CSParseri(&flows[id].IntArriv, &flows[id].PktSize, flows[id].IntArrivDistro,
                              flows[id].PktSizeDistro);
                    flows[id].l4Proto = L4_PROTO_UDP;
                    h++;
                    argc--;
                    break;
                case L7_PROTO_QUAKE:
                    QuakeParser(&flows[id].IntArriv, &flows[id].PktSize, flows[id].IntArrivDistro,
                                flows[id].PktSizeDistro);
                    flows[id].l4Proto = L4_PROTO_UDP;
                    h++;
                    argc--;
                ...
                case LX_ERROR_BYTE:
                    char temp[sizeof("What is  ?") + sizeof(argv[h])];
                    ReportErrorAndExit("General parser",
                                       strcat(strcat(strcpy(temp, "What is "), argv[h]), " ?"),
                                       programName, id);
                    break;
            }
        }
    ...
    if (flag != flows[id].dstAddrSpecify) {
        ...
        if (flows[id].SigDestHost->ai_family == PF_INET) {
            ...
            getnameinfo(flows[id].SigDestHost->ai_addr, flows[id].SigDestHost->ai_addrlen, buffer,
...
void *signalManager(void *id) {
    ...
    for (;;) {
        ...
        while (pending > 0) {
            ...
            if (size <= 0) {
                ReportErrorAndExit("signalManager",
                                   "Receiver has shut down the connection gracefully", programName,
                                   0);
            }
            ...
            switch (type) {
                ...
                default:
                    ReportErrorAndExit("signalManager", "Got unknown message from receiver",
                                       programName, 0);
                    break;
            }
            fflush(stdout);
        ...
        while(available > 0) {
            ...
            switch (msg.code) {
                ...
                case MSG_SM_ENDFLOW:
                    ...
                    PRINTD(1,
                           "signalManager: sending request on signalling channel to close flow #%d\n",
                           msg.flowId);
...
identifySignalManager(int flowId, int *chanId, struct addrinfo *&DestHost, bool &dstAddrSpecify) {
    ...
    if ((checkChanID == false) && (okToNewSignalingChan == true)) {
...
#ifdef UNIX
        createTransportChan(signalSock, DestHost, dstAddrSpecify, flows[flowId].SigSrcHost,
                            flows[flowId].SigSrcPort, flows[flowId].SigInterface);
#else
        ...
        if ((passiveMode == true) && (multiFlows == 1)) {
            ...
            getnameinfo(DestHost->ai_addr, DestHost->ai_addrlen, globalSigAddr, INET6_ADDRSTRLEN,
        ...
        if (logServer == 1) {
            if (sendLog(signalSock, serverLogReceiver, protoTx_ServerLogReceiver, logFileReceiver) <
                0) {
        ...
        *chanId = 0;
        ...
        if ((passiveMode == true) && (multiFlows == 1)) {
            ...
            PRINTD(1,
                   "identifySignalManager: Using a old signaling channel,\n\tchanId=%d,\n\tflow_number=%d,\n\tglobal_signaling_address=%s\n\n",
...
void *flowSender(void *para) {
    ...
    if (flows[id].Delay) {
        ...
        flows[id].FlowStartTime.tv_sec = RifTime.tv_sec + flows[id].Delay / 1000 +
                                         (flows[id].FlowStartTime.tv_usec = RifTime.tv_usec +
                                                                            (flows[id].Delay %
                                                                             1000) * 1000) /
                                         1000000;
    ...
    if (SrcAddrSpecify == true) {
        if (SrcHost->ai_family == PF_INET6) {
            getnameinfo(SrcHost->ai_addr, SrcHost->ai_addrlen, dotIpV6, INET6_ADDRSTRLEN, NULL, 0,
                        NI_NUMERICHOST);
        } else {
            getnameinfo(SrcHost->ai_addr, SrcHost->ai_addrlen, dotIpV4, INET_ADDRSTRLEN, NULL, 0,
                        NI_NUMERICHOST);
        }
    }
    ...
    sock = socket(SrcHost->ai_family, socktype, prototype);
//    sock = socket(AF_INET, socktype, prototype);
#endif
    if (sock < 0) {
        perror("flowSender");
    ...
    if (SrcPortSpecify || SrcAddrSpecify) {
        ...
        if ((passiveMode == true) && ((SrcHost->ai_family) != (DestHost->ai_family)) &&
            (SrcHost->ai_family == PF_INET)) {
            char oldAddr[INET_ADDRSTRLEN + 1];
            char newAddr[INET6_ADDRSTRLEN + 1];


            getnameinfo(SrcHost->ai_addr, SrcHost->ai_addrlen, oldAddr, INET_ADDRSTRLEN, NULL, 0,
                        NI_NUMERICHOST);
    ...
    if (DSByte && (DestHost->ai_family == AF_INET) &&
        setsockopt(sock, SOL_IP, IP_TOS, (char *) &DSByte, sizeof(DSByte)) < 0)
...
#else
            if (recvfrom(sock, &ch, 1, 0, flows[id].DestHost->ai_addr,
                         &flows[id].DestHost->ai_addrlen) < 0) {
                PRINTD(1, "\nflowSender: Error:%s\n", strerror(errno));
                ReportErrorAndExit("flowSender", "error in recvfrom(). (Passive Mode)", programName,
                                   id);
            }
#endif
    ...
    do {
        ...
        if (flows[id].dimPayloadBuffer != 0) {
            ...
            if ((indexBuffer + byteToCopy) <= flows[id].dimPayloadBuffer) {
                ...
                memcpy(ptrToTruePayload,
                       (void *) ((long int) flows[id].ptrPayloadBuffer + indexBuffer),
                       byteToCopy);
            ...
            if ((logging) && (meter == METER_OWDM)) {
                ...
                if (logremoto == 0) {
                    ...
                    writeInBufferStandard(&infos[count], htonl(id), htonl(seqNum), HelpSrcAddress,
                                          HelpDstAddress, tmpPort_SrcPort, tmpPort_DstPort, time,
                                          time,
        ...
        if (flows[id].bursty) {
            ...
            PRINTD(1,"flowSender: On Duration = %lf\n",OnDuration);
              ...
              || (Ticker.lastTime.tv_sec == end_time.tv_sec &&
                  Ticker.lastTime.tv_usec < end_time.tv_usec))
             && ((seqNum - 1) < numPacket) && (byteSent < byteToSend));
                    ...
                    if (logging && size_r > 0) {
                        if (logremoto == 0) {
                            ...
                            writeInBufferStandard(&infos[count], *(uint32_t *) payload,
                                                  *(uint32_t *) ptrSeqNum, HelpSrcAddress,
                                                  HelpDstAddress,
                                                  tmpPort_SrcPort, tmpPort_DstPort, time1, time2,
                                                  net_TimeUsec,
...
void printHelp() {
    ...
    cout << "\n   Log options (log_opts):\n\n"
            ...
            "     -l [logfile]              Generate sender-side log file (default: "
         << DefaultLogFile << ").\n\n"

                              "     -L [log_server_address]   Generate sender-side log file on a remote ITGLog instance\n"
                              "        [logging_protocol]     (default: <" << DEFAULT_LOG_IP
         << "> <" << invFindL4Proto(DEFAULT_PROTOCOL_TX_LOG) << ">).\n\n"

                                                                "     -x [receiver_logfile]     Ask ITGRecv to generate receiver-side log file (default: "
         << DefaultRecvLogFile << ").\n\n"
         ...
         << "> <" << invFindL4Proto(DEFAULT_PROTOCOL_TX_LOG) << ">).\n\n"
                                                                ...
                                                                "     -q <log_buffer_size>      Number of packets to push to the log at once (default: "
         << logbuffer_size << ").\n\n";
    ...
    cout << "\n  Flow options (flow_opts):\n\n"
            ...
            "     -t  <duration>          Set the generation duration in ms (default: "
         << DefaultDuration << " ms).\n\n"
                               ...
                               "     -d  <delay>             Start the generation after the specified delay in ms (default: "
         << DefaultDelay << " ms).\n\n"

                            "     -b <DS_byte>            Set the DS byte for QoS tests (default: "
         << DefaultDSByte << ").\n\n"

                             "     -f <TTL byte>           Set the IP Time To Live (default:  64).\n\n"

                             "     -a  <dest_address>      Set the destination address (default: "
         << DefaultDestIP << ").\n\n"

                             "     -sa <src_address>       Set the source address (default: Set by O.S.).\n\n"

                             "     -rp <dest_port>         Set the destination port (default: "
         << DEFAULT_PORT << ").\n\n"
    ...
    cout
            << "     -p <payload_metadata>   Select the metadata sent in the payload of each packet (default: 2).\n"
               ...
               "     -T <protocol>           Layer 4 protocol (default: "
            << invFindL4Proto(L4_PROTO_UDP) << "):\n"
    ...
    cout << "\n  Inter-departure time options (idt_opts):\n\n"
            ...
            "     -C  <rate>                 Constant (default: " << DefaultPktPerSec
         << " pkts/s).\n\n"
...
#endif
{
    ...
    if (passiveMode == false) {
        ...
        if ((newSock = accept(signalSock, TmpAddress->ai_addr,
                              (socklen_t *) &(TmpAddress->ai_addrlen))) < 0) {
        ...
        if (dstAddrSpecify == false) {
            ...
            if (TmpAddress->ai_family == PF_INET) {
                ...
                getnameinfo(TmpAddress->ai_addr, TmpAddress->ai_addrlen, buffer, INET_ADDRSTRLEN,
                            NULL, 0, NI_NUMERICHOST);
            } else if (TmpAddress->ai_family == PF_INET6) {

                buffer = new char[INET6_ADDRSTRLEN];

                getnameinfo(TmpAddress->ai_addr, TmpAddress->ai_addrlen, buffer, INET6_ADDRSTRLEN,
                            NULL, 0, NI_NUMERICHOST);
            }
...
void createRemoteLogFile(struct addrinfo *logHost, BYTE protocolLog) {
        ...
        case L4_PROTO_UDP :
            ...
            if (logSock < 0)
                ReportErrorAndExit("createRemoteLogFile",
                                   "socket DATAGRAM - Cannot create socket logSock", programName,
                                   0);
            break;
        case L4_PROTO_TCP :
            ...
            if (logSock < 0)
                ReportErrorAndExit("createRemoteLogFile",
                                   "socket STREAM - Cannot create socket logSock", programName, 0);
            break;
        default :
            break;
    }
    ...
    if (recv(logSockSignaling, (char *) &port, sizeof(port), 0) < 0)
        ReportErrorAndExit("createRemoteLogFile", "Cannot receive on signaling socket", programName,
                           0);
    port = ntohl(port);
    ...
    if (connect(logSock, logHost->ai_addr, logHost->ai_addrlen) < 0)
        ReportErrorAndExit("createRemoteLogFile", "Error into connect with logsender!", programName,
                           0);
}
...
void createSignalingLogChannel(BYTE protocolLog) {
    ...
    if (logSockSignaling < 0)
        ReportErrorAndExit("createSignalingLogChannel", "Cannot create socket logSockSignaling!",
                           programName, 0);
    ...
    if (send(logSockSignaling, (char *) &signal, sizeof(signal), 0) < 0)
        ReportErrorAndExit("createSignalingLogChannel", "Cannot send infos to LogServer !",
                           programName, 0);

    PRINTD(1,
           "createSignalingLogChannel: Sent to LogServer infos for creating signaling channel\n");
}
