...
static Real cdf_chisq_large(Real x, int df, bool upper) {
    ...
    for (;;) {
        df -= 2;
        if (df == 1) {
            sum += 2.0 * cdfnml(-sqrt(x));
            return upper ? sum : 1.0 - sum;
        }
        Real ratio = df / x;
        term *= ratio;
        sum += term;
...
static Real cdf_chisq_very_small(Real x, int df) {
    Real df2 = df / 2.0;
    Real x2 = x / 2.0;
    Real term = 2.0 * exp(df2 * log(x2) - ln_gamma(df2));
    Real sum = term / df;
    if (sum == 0) return 0.0;
    for (int i = 1;; i++) {
        df += 2;
