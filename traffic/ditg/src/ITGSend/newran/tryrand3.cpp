...
static void MyQuickSortAscending(Real *first, Real *last, int depth) {
    for (;;) {
        ...
        if (f > centre) {
            MyQuickSortAscending(l + 1, last, depth);
            last = f - 1;
        }
        else {
            MyQuickSortAscending(first, f - 1, depth);
            first = l + 1;
        }
    }
}
...
void ChiSquaredGOFTest(const char *name, int *Observed, Real *Prob, int N, int n) {
    ...
    if (fabs(ToGo) >= 0.01) cout << "chi-squared program fails " << endl;
        ...
        ChiSquaredTestUpper CSTU(name, CS, df - 1);
        CSTU.DoTest();
    }
...
void TestBinomial(int N, Real p, int n) {
    ...
    Real ln_q = log(q);
    int *obs = new int[N + 1];
    if (!obs) Throw(Bad_alloc());
    Real *prob = new Real[N + 1];
    if (!prob) Throw(Bad_alloc());
    int i;
    for (i = 0; i <= N; i++) {
        obs[i] = 0;
...
void TestPoisson(Real mu, int n) {
    ...
    for (i = 0; i <= N; i++) {
        obs[i] = 0;
        prob[i] = exp(i * ln_mu - mu - ln_gamma(i + 1));
    }
    for (i = 0; i < n; i++) {
        int b = (int) (X.Next());
