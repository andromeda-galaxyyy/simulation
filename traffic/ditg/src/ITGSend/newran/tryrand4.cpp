...
void test_perm(int N, int NN, unsigned int M) {
    ...
    last = 0;
    count = 0;         // set the arrays to zero
    ...
    for (i = 1; i <= M; i++)              // iterate over total number of trials
    {
        ...
        Array<int> check(0, NN - 1);
        check = 0;  // for checking no repeats
    ...
    if (N < NN) {
        ...
        NormalTestTwoSided repeats_test("Repeats", sum2, (M - 1) * (N * N / (double) NN),
                                        square((double) (N * (NN - N)) / (double) NN) * (M - 1) /
                                        (NN - 1));
        repeats_test.DoTest();
    ...
    if (N > 1 && N < NN) {
        // pairs of balls
        double CS = 0;
        double ev = (double) M * N * (N - 1) / NN / (NN - 1);
        for (int i = 0; i < NN; i++)
            for (int j = i + 1; j < NN; j++)
                CS += square(table2(i, j) - ev);
        CS /= ev;
        ev = (NN - N) * (NN + N - 1) / 2.0;
        double v = (double) (M - 1) * square(NN - N) *
                   (-3 + 6 * N - 3.0 * N * N + 2 * NN - 6.0 * N * NN +
