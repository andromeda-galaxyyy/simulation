...
void test1(double N) {
    ...
    for (j = 0; j < N; ++j) {
        ...
        sumsq += v * v;
        ar1 += v * (last - 0.5);
        int k = (int) floor(u * 256);
        ++chi1(k);
        int m = (int) floor(u * 65536);
        ++chi2(m);
        int a = (int) floor(u * 16);
        ++chi0(a);
        if (j > 0) {
            int b = (int) floor(last * 16);
            ++chi1x(a + 16 * b);
            int l = (int) floor(last * 256);
            ++chi2x(k + 256 * l);
        }
        last = u;
    ...
    NormalTestTwoSided test_mean("Mean", sum, 0.0, N / 12.0);
    test_mean.DoTest();
    NormalTestTwoSided test_var("Variance", sumsq, N / 12.0, N / 180.0);
    test_var.DoTest();
    NormalTestTwoSided test_cov("AutoCov 1", ar1, 0.0, N / 144.0);
    test_cov.DoTest();

    ChiSquaredTestTwoSided Chi_4("Chi 4", chisq_test(chi0, 16), 15);
    Chi_4.DoTest();
    ChiSquaredTestTwoSided Chi_8("Chi 8", chisq_test(chi1, 256), 255);
    Chi_8.DoTest();
    ChiSquaredTestTwoSided Chi_16("Chi 16", chisq_test(chi2, 65536), 65535);
    Chi_16.DoTest();
    ...
    ChiSquaredTestTwoSided MM_0_1_16("MM 0-1-16",
                                     chisq_test(crawl16, 65536) - chisq_test(crawl15, 32768),
                                     32768);
    MM_0_1_16.DoTest();
...
static short int SumOfBits[] = {                   // sum of bits in a byte
        ...
        5,
        1, 2, 2, 3, 2, 3, 3, 4, 2, 3, 3, 4, 3, 4, 4, 5, 2, 3, 3, 4, 3, 4, 4, 5, 3, 4, 4, 5, 4, 5, 5,
        6,
        1, 2, 2, 3, 2, 3, 3, 4, 2, 3, 3, 4, 3, 4, 4, 5, 2, 3, 3, 4, 3, 4, 4, 5, 3, 4, 4, 5, 4, 5, 5,
        6,
        2, 3, 3, 4, 3, 4, 4, 5, 3, 4, 4, 5, 4, 5, 5, 6, 3, 4, 4, 5, 4, 5, 5, 6, 4, 5, 5, 6, 5, 6, 6,
        7,
        1, 2, 2, 3, 2, 3, 3, 4, 2, 3, 3, 4, 3, 4, 4, 5, 2, 3, 3, 4, 3, 4, 4, 5, 3, 4, 4, 5, 4, 5, 5,
        6,
        2, 3, 3, 4, 3, 4, 4, 5, 3, 4, 4, 5, 4, 5, 5, 6, 3, 4, 4, 5, 4, 5, 5, 6, 4, 5, 5, 6, 5, 6, 6,
