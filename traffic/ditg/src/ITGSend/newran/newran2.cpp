...
ChiSq::ChiSq(int df, Real noncen) {
    if (df <= 0 || noncen < 0.0) Throw(Logic_error("Newran: illegal parameters"));
        ...
        if (df == 2) {
            ...
            version = 2;
            c1 = new Gamma2(0.5 * df);
        }
    } else if (df == 2) {
        version = 3;
        c1 = new ChiSq1(noncen / 2.0);
    }
    else if (df == 3) {
        version = 4;
        c1 = new Exponential();
        c2 = new ChiSq1(noncen);
    }
    else {
        version = 5;
        c1 = new Gamma2(0.5 * (df - 1));
        c2 = new ChiSq1(noncen);
    }
...
Real Pareto::Next() { return pow(RNG->Next(), RS); }
...
Stable::Stable(Real a, Real bx, Notation n)
    ...
    if (notation == Kalpha) {
        ...
        Throw(Logic_error("Newran: Stable notation not recognised"));
}
...
Real Stable::Next() {
    ...
    Real Sprime = (1.0 + eps * d) * 2.0 * ((a - b) * (1.0 + a * b)
                                           - phiby2 * tau * bb * (b * a2 - 2.0 * a)) / (a2 * b2p) +
                  tau * d;
...
ExtReal Stable::Mean() const {
    if (alpha < 1.0)
        return (bprime == 1.0) ? PlusInfinity :
...
{
    int i = (int) (n * RNG->Next());
    if (RNG->Next() < p[i]) i = ialt[i];
...
Real ln_gamma_asymp(Real z) {
    ...
    return
            ...
            + (1.0L / 12.0L + (-1.0L / 360.0L + (1.0L / 1260.0L - 1.0L / 1680.0L / z2) / z2) / z2) /
              z;
}
                                                  ...
                                                  + z1 * (0.01734306198444913971L / 6
                                                          + z1 * (-0.00834927738192282684L / 7
                                                                  ...
                                                                  z1 * (0.00407735619794433938L / 8
                                                                        + z1 *
                                                                          (-0.00200838282608221442L /
                                                                           9
                                                                           + z1 *
                                                                             (0.00099457512781808534L /
                                                                              10
                                                                              + z1 *
                                                                                (-0.00049418860411946456L /
                                                                                 11
                                                                                 + z1 *
                                                                                   (0.00024608655330804830L /
                                                                                    12
                                                                                    + z1 *
                                                                                      (-0.00012271334757848915L /
                                                                                       13
                                                                                       + z1 *
                                                                                         (0.00006124813505870483L /
                                                                                          14
                                                                                          + z1 *
                                                                                            (-0.00003058823630702049L /
                                                                                             15)))))))))))))));
}
...
Real ln_gamma(Real z, int &sign) {
    ...
    if (z >= 2.1) {
        sign = 1;
        return ln_gamma_gt_1(z);
    }
    if (z >= 1.9) {
        sign = 1;
        return ln_gamma_near_2(z);
    }
    if (z >= 1.1) {
        sign = 1;
        return ln_gamma_gt_1(z);
...
ExtReal AddedRandom::Variance() const { return rv1->Variance() + rv2->Variance(); }
...
ExtReal SubtractedRandom::Variance() const { return rv1->Variance() + rv2->Variance(); }
...
ExtReal MultipliedRandom::Variance() const {
    ExtReal e1 = square(rv1->Mean());
    ExtReal e2 = square(rv2->Mean());
    ExtReal v1 = rv1->Variance();
    ExtReal v2 = rv2->Variance();
...
void Random::load(int *, Real *, Random **) { Throw(Logic_error("Newran: illegal combination")); }
...
int AddedSelectedRandom::nelems() const { return rv1->nelems() + rv2->nelems(); }

void AddedSelectedRandom::load(int *i, Real *probs, Random **rvx) {
    rv1->load(i, probs, rvx);
    rv2->load(i, probs, rvx);
    delete this;
}
...
Real VariPoisson::Next(Real mu) {
    if (mu <= 0) {
        if (mu == 0) return 0;
            ...
            Throw(Logic_error("Newran: illegal parameters"));
    }
    if (mu < 10) return Next_very_small(mu);
...
int VariPoisson::iNext(Real mu) {
    Tracer et("VariPoisson::iNext ");
    return Round(Next(mu));
}
...
Real VariBinomial::Next_small(int n, Real p) {
    ...
    int k2 = k_start;
    for (;;) {
        t1 *= r * (Real) (n - k1) / (Real) (k1 + 1);
        ++k1;
        s += t1;
        if (s >= u) return k1;
        if (k2) {
            --k2;
            t2 *= (Real) (k2 + 1) / (Real) (n - k2) / r;
            s += t2;
            if (s >= u) return k2;
...
int VariBinomial::iNext(int n, Real p) {
    Tracer et("VariBinomial::iNext ");
    return Round(Next(n, p));
}
