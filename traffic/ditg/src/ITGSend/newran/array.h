...
GeneralArray<T>::GeneralArray(int size) : sz(size) {
    if (sz > 0) {
        data = new T[sz];
        if (data == 0) Throw(Bad_alloc("Array: out of space"));
    }
    else data = 0;
}
...
template<class T>
GeneralArray<T>::~GeneralArray() { delete[] data; }               // should call individual destructors

/// Element of an array.
template<class T>
inline T &GeneralArray<T>::El(int i) {
    if (!(i >= 0 && i < sz))
        Throw(Logic_error("Array: subscript error"));
    return data[i];
}
...
GeneralArray<T> GeneralArray<T>::operator+(const GeneralArray<T> &A) const {
    int l = length();
    if (l != A.length()) Throw(Logic_error("Array: incompatible subscripts"));
    const T *s1 = base();
    const T *s2 = A.base();
...
T_Array<T> Array<T>::operator+(const Array<T> &A) const {
    int lb = first();
    if (A.first() != lb) Throw(Logic_error("Array: incompatible subscripts"));
    int l = this->length();
    if (l != A.length()) Throw(Logic_error("Array: incompatible subscripts"));
    const T *s1 = this->base();
    const T *s2 = A.base();
