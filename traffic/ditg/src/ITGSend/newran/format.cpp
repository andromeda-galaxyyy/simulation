...
OstreamWithFormat &OstreamWithFormat::operator<<(const char *value) {
    f.StringPrint(os, value, strlen(value));
    return *this;
}
...
void Format::StringPrint(ostream &os, const char *value, int size) const {
    ...
    if (alignment_val == Format::LEFT) {
        s1 = 0;
        s2 = spaces;
    }
    else if (alignment_val == Format::RIGHT) {
        s1 = spaces;
        s2 = 0;
    }
    else {
...
void Format::DecFig(ostream &os, double value, int ndec,
    ...
    if (variant1) {
        if (li < 0) {
            DecFig(os, value, ndec + li, force_decimal);
            return;
        }
    } else {
        if (li < 1) {
            DecFig(os, value, ndec + li - 1, force_decimal);
            return;
        }
    }
...
void Format::Scientific(ostream &os, double value, int nsig) const {
    ...
    if (lv >= 100 || lv <= -100) { if (lv >= 1000 || lv <= -1000) np = 4; else np = 3; }
    else np = 2;
    ...
    if (w > max_width_val) {
        ...
        if (overflow_policy_val == Format::HASH) {
            ErrorOut(os);
            return;
        }
