...
StrRep *String::GetStrRep() const { return px->GetStrRep(&(StrBase *&) px); }
...
char String::operator[](uint pos) const {
    ...
    if (pos >= p->size()) {
        if (pos == p->size()) return 0;
            ...
            Throw(Out_of_range("string index error\n"));
    }
    return (p->s)[pos];
}
...
String &String::my_append(const StrBase &sb) {
    ...
    if (rlen > capacity() || refcount() != 0) {
        ...
        StrRep *oldp = p;
        p = new StrRepMult(rlen);
        memcpy(p->s, oldp->s, sz);
        oldp->Drop();
        px = p;
    }
    char *target = p->s + sz;
...
String &String::assign(const char *s, uint n) { return my_assign(CharSeq((char *) s, n)); }

String &String::assign(const char *s) { return my_assign(CharSeq((char *) s)); }

String &String::assign(uint n, char c) { return my_assign(CharRepeated(n, c)); }
...
String &String::my_insert(uint pos, const StrBase &sb) {
    ...
    if (HasCapacity() && sz + rlen <= capacity()) {
        ...
        StrRep *p = new StrRepMult(sz + rlen);
        px = p;
        char *target = p->s;
        memcpy(target, oldp->s, pos);
        target += pos;
        sb.Load(target);
...
String &String::insert(uint pos1, const String &str) { return my_insert(pos1, *(str.px)); }
...
String &String::insert(uint pos, uint n, char c) { return my_insert(pos, CharRepeated(n, c)); }
...
String &String::my_replace(uint pos, uint n, const StrBase &sb) {
    ...
    else if (HasCapacity() && sz + rlen <= capacity()) {
        // must allow for possibility that sb is part of *this
        char *target = oldp->s + pos;
...
String &String::replace(uint pos1, uint n1, const String &str, uint pos2,
                        uint n2) { return my_replace(pos1, n1, CharSeq(str, pos2, n2)); }
...
uint String::copy(char *s, uint n, uint pos) const {
    ...
    rlen -= pos;
    if (rlen > n) rlen = n;
    memcpy(s, p->s + pos, rlen);
    return rlen;
}

void String::swap(String &str) {
    StrBase *oldp = px->UnProtect();
    px = str.px->UnProtect();
    str.px = oldp;
}

const char *String::c_str() const { return px->NullTerminate(&(StrBase *&) px); }
...
uint String::find(const CharSeq &str, uint pos) const {
    ...
    const char *start = GetData();
    const char *newstart = start + pos;
...
uint String::find(const String &str, uint pos) const { return find(*(str.GetStrRep()), pos); }

uint String::find(const char *s, uint pos, uint n) const {
    return find(CharSeq((char *) s, n), pos);
}

uint String::find(const char *s, uint pos) const { return find(CharSeq((char *) s), pos); }
...
uint String::rfind(const String &str, uint pos) const { return rfind(*(str.GetStrRep()), pos); }

uint String::rfind(const char *s, uint pos, uint n) const {
    return rfind(CharSeq((char *) s, n), pos);
}

uint String::rfind(const char *s, uint pos) const { return rfind(CharSeq((char *) s), pos); }
...
uint String::find_first_of(const char c, uint pos) const { return find(c, pos); }
...
uint String::find_last_of(const char c, uint pos) const { return rfind(c, pos); }
...
String String::substr(uint pos, uint n) const { return String(*this, pos, n); }
...
int String::compare(const String &str) const { return compare(0, npos, CharSeq(str)); }
...
int String::compare(const char *s) const { return compare(0, npos, CharSeq((char *) s)); }
...
ostream &operator<<(ostream &os, const String &str) { return os << str.c_str(); }
...
istream &getline(istream &is, String &str, char delim) {
    ...
    while (((token = is.get()) != EOF) && (token != delim)) { str += char(token); }
    if (token == EOF) { is.clear(ios::eofbit | is.rdstate()); }
...
void StrSum::Load(char *&ps) const {
    lhs->Load(ps);
    rhs->Load(ps);
}

void StrSumLC::Load(char *&ps) const {
    *ps++ = lhs;
    rhs->Load(ps);
}

void StrSumRC::Load(char *&ps) const {
    lhs->Load(ps);
    *ps++ = rhs;
}

void CharSeq::Load(char *&ps) const {
    memmove(ps, s, sz);
    ps += sz;
}
...
void CharRepeated::Load(char *&ps) const {
    memset(ps, c, n);
    ps += n;
}
...
void StrBase::WithCapacity(StrBase **px) {
    uint sz1 = size();
    StrRepCap *p = new StrRepCap(sz1, sz1);
    char *target = p->s;
    Load(target);
    (*px)->Drop();
    *px = p;
...
StrBase *StrRepMult::Clone(StrBase **) {
    refcnt++;
    return this;
}
