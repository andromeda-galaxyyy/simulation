...
public:
    ...
    enum FT {
        SIG_FIGS, DEC_FIGS, SCIENTIFIC, INTEGER
    };
    enum POS {
        PLUS, SPACE, NX
    };
...
public:
    ...
    OstreamWithFormat &operator<<(const String &value) {
        f.StringPrint(os, value.data(), value.size());
        return *this;
    }

    OstreamWithFormat &operator<<(int value) {
        f.IntegerPrint(os, (double) value);
        return *this;
    }
    ...
    OstreamWithFormat &operator<<(double value) {
        f.FormatOut(os, value);
        return *this;
    }

    OstreamWithFormat &operator<<(float value) { return operator<<((double) value); }
    ...
    OstreamWithFormat &operator<<(char value) {
        os << value;
        return *this;
    }
...
inline OstreamWithFormat
operator<<(OstreamWithFormat &osx, const Format &fx) { return OstreamWithFormat(osx.os, fx); }
