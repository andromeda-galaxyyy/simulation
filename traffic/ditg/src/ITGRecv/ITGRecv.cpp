...
int ipv6ToIfIndex(struct in6_addr *ipv6_addr) {
    ...
    if ((f = fopen("/proc/net/if_inet6", "r")) != NULL) {
        while (fscanf(f, "%4s%4s%4s%4s%4s%4s%4s%4s %02x %02x %02x %02x %20s\n",
                      addr6p[0], addr6p[1], addr6p[2], addr6p[3], addr6p[4], addr6p[5], addr6p[6],
                      addr6p[7],
...
void terminate(int sign) {
    if (flagTerm == NO_TERMINATE) {
        ...
        if (MUTEX_THREAD_RELEASE(mutexLogRem) < 0) {
            ...
            flagTerm = ERROR_TERMINATE;
            reportErrorAndExit("terminate", "MUTEX_THREAD_RELEASE",
                               "Cannot release Remote Log Mutex ");
        }
#ifdef WIN32
    ...
    while (1) {
        ...
        if (FD_ISSET(fd, &activeSet)) {
            ...
            if (type == TSP_SEND_NAME_LOG) {
                ...
                if (logRemote != 1) {
                    createRemoteLogFile(logHost, nameFileLog, protocolLog, logSockSignaling,
                                        logSock);
                    logRemote = 2;
                } else {
                    logSockSignaling = globaleLogSockSignaling;
...
#endif
    cout << "    -Sp <port>               Signaling channel port number (default: "
         << DEFAULT_PORT_SIGNALING << ").\n\n"

                                      "    -l [logfile]             Enable logging to file (default filename: "
         << DEFAULT_LOG_FILE << ").\n\n"

                                "    -L                       Enable remote logging to a remote ITGLog server.\n"
                                "       [a:<address>]         ITGLog server address (default: "
         << DEFAULT_LOG_IP << ").\n"
                              "       [p:<port>]            ITGLog server signaling port (default: "
         << DEFAULT_LOG_PORT_SIGNALING << ").\n"
                                          "       [P:<protocol>]        ITGLog server transport protocol (default: "
...
void
recvFlowLog(int newSockSignaling, struct addrinfo &logHost, BYTE &protocolLog, char logFile[]) {
...
int typeParser(BYTE type, int &numFlow, int newSockSignaling,
               memChannel flowIdNum[], paramThread paraThread[], pthread_t hThr[],
               int rPipe[], char *fileLog, int logSock, int logSockSignaling,
               struct addrinfo *logHost)
#endif
{
        ...
        if (flowPosition < MAX_NUM_THREAD) {
            ...
            if (passiveMode == true) {
                ...
                if (recv(newSockSignaling, (char *) &portForPssv, sizeof(uint16_t), 0) < 0) {
                    reportErrorAndExit("typeParser", "recv - type = TSP_SEND_FLOW(3",
                                       "Cannot receive portForPssv on newSockSignaling");
                }
                ...
                if (recv(newSockSignaling, (char *) &srcAddrSpecify, sizeof(BYTE), 0) < 0) {
                    reportErrorAndExit("typeParser", "recv - type = TSP_SEND_FLOW(3",
                                       "Cannot receive srcAddrSpecify on newSockSignaling");
                }
            ...
            paraThread[flowPosition].destHost.ai_family = (l3Proto == L3_PROTO_IPv4) ? PF_INET
                                                                                     : PF_INET6;
            ...
            if (!(srcAddrSpecify == 0x00 && passiveMode == true)) {
                ...
                if (paraThread[flowPosition].destHost.ai_family == PF_INET) {
                    struct sockaddr_in *destAddr = (struct sockaddr_in *) calloc(1,
                    ...
                    struct sockaddr_in6 *destAddr = (struct sockaddr_in6 *) calloc(1,
                ...
                if (passiveSender->ai_family == PF_INET) {
                    ...
                    paraThread[flowPosition].destHost.ai_addr = (struct sockaddr *) malloc(
                            sizeof(struct sockaddr_in));
                    memcpy(paraThread[flowPosition].destHost.ai_addr, passiveSender->ai_addr,
                           passiveSender->ai_addrlen);
                    ...
                    memcpy(&destAddr->sin6_addr,
                           &((struct sockaddr_in6 *) passiveSender->ai_addr)->sin6_addr,
                           4 * sizeof(in_addr_t));
                    destAddr->sin6_family = AF_INET6;
                    destAddr->sin6_flowinfo = 0;
...
#else
            if ((flowIdNum[flowPosition].l4Proto == L4_PROTO_TCP) &&
                (paraThread[flowPosition].payloadLogType != PL_NONE)) {
#endif
                paraThread[flowPosition].preambleSize =
                        paraThread[flowPosition].preambleSize + sizeof(uint32_t);
            }
...
//                                       "Cannot send TSP_ACK_SEND_FLOW message to sender");
            ...
            if (globalBindAddr != NULL) {
                ...
                if (globalBindAddr->ai_family == PF_INET) {
                    ...
                    paraThread[flowPosition].destHost.ai_addr = (struct sockaddr *) malloc(
                            sizeof(struct sockaddr_in));
                    memcpy(paraThread[flowPosition].destHost.ai_addr, globalBindAddr->ai_addr,
                           globalBindAddr->ai_addrlen);
            ...
            switch (flowIdNum[flowPosition].l4Proto) {
                case L4_PROTO_UDP:
                    ...
                    PRINTD(1,
                           "typeParser: Return value CREATE_THREAD udpSock hThr[flowPosition] : %lu \n",
                ...
                case L4_PROTO_TCP:
                    ...
                    PRINTD(1,
                           "typeParser: Return value CREATE_THREAD tcpSock hThr[flowPosition] : %lu \n",
                           (unsigned long int) hThr[flowPosition]);
                    break;
#ifdef SCTP
                case L4_PROTO_SCTP:
                ...
                case L4_PROTO_ICMP:
                    ...
                    PRINTD(1,
                           "typeParser: Return value CREATE_THREAD icmpSock hThr[flowPosition] : %lu \n",
                           (unsigned long int) hThr[flowPosition]);
                    break;
                default:
                    break;
        ...
        if (k < MAX_NUM_THREAD) {
            ...
            if ((logCheck == 1) || (logCheck == 2))
                ...
                if ((size = sendto(logSock, (char *) paraThread[k].addressInfos,
                                   paraThread[k].count * sizeof(struct info), 0, logHost->ai_addr,
                                   logHost->ai_addrlen)) < 0)
...
//                                   "Cannot send ack closed flow id on newSockSignaling");
        ...
        *(int *) ptrFlowId = ntohl(*(int *) ptrFlowId);
        ...
        PRINTD(1,
               "typeParser: Received TSP_ERR_MSG_1(14) message : authentication not successful\n");
        return -1;
...
void parserRecv(int argc, char *argv[]) {
    ...
    while (argc > 0) {
        ...
        switch (argv[0][1]) {
            ...
            case 'n':
                if (argc < 2)
                    reportErrorAndExit("parserRecv", "general parser",
                                       "Invalid number of \"Hole Punching\" packets");
                tmp = atoi(argv[1]);
                if (tmp < 0)
                    reportErrorAndExit("parserRecv", "general parser",
                                       "Invalid number of \"Hole Punching\" packets");
...
int main(int argc, char *argv[]) {
    ...
    if (passiveMode == false) {
        ...
        if (SigInterface != NULL) {
            ...
            if (setsockopt(sockSignaling, SOL_SOCKET, SO_BINDTODEVICE, SigInterface,
                           strlen(SigInterface)) < 0) {
        ...
        if (SigInterface != NULL) {
            ...
            if (setsockopt(sockSignaling, SOL_SOCKET, SO_BINDTODEVICE, SigInterface,
                ...
                fflush(stdout);
                    ...
                    setsockopt(sockSignaling, SOL_SOCKET, SO_DONTROUTE, &dontRoute, sizeof(int)) <
                    0) {
