...
class Parser:
	...
	def parse(self) -> Dict[Tuple, List[Tuple]]:
		...
		if not pathlib.Path(self.file).exists(): raise Exception
...
class TCPParser(Parser):
	...
	def parse(self) -> Dict[Tuple, List[Tuple]]:
		...
		logger.debug("There is {} flows (maybe bidirectional)".format(len(packets)))
		...
		logger.info("There is {} disctinct flows".format(len(res)))
