...
DelayInstance = namedtuple("DelayInstance", field_names=[
                           "min", "max", "mean", "std", "label"])
LossInstance = namedtuple(typename="LossInstance",
                          field_names=["loss", "label"])
...
loss_dirs = ["/tmp/loss01", "/tmp/loss02",
             "/tmp/loss04", "/tmp/loss06"]
...
def plot_delay(delay_instances: List[DelayInstance], fn="/tmp/delay.png"):
    ...
    plt.boxplot([delay5_data, delay10_data, delay20_data, delay50_data],
                positions=[0, 1, 2, 3], labels=["5", "10", "20", "50"])
    plt.yticks(np.arange(0, 60, 5))
    plt.ylabel("估算值(ms)", fontsize=14)
    plt.xlabel("真实值(ms)", fontsize=14)
...
def plot_loss(loss_instances: List[LossInstance], fn="/tmp/loss.png"):
    ...
    # loss01_data = np.asarray([l.loss for l in loss01_instances if l.loss>0.05 and l.loss<0.15])
    # debug("loss 0.1 {}".format(len(loss01_data)))
    # loss02_data = np.asarray([l.loss for l in loss02_instances if l.loss>0.15 and l.loss<0.25])
    # loss04_data = np.asarray([l.loss for l in loss04_instances if l.loss>0.35 and l.loss<0.45])
    # loss06_data = np.asarray([l.loss for l in loss06_instances if l.loss>0.55 and l.loss<0.65])
    ...
    plt.boxplot([loss01_data,loss02_data,loss04_data,loss06_data],positions=[0,1,2,3],labels=["0.1","0.2","0.4","0.6"])

    plt.yticks(np.arange(0,1,0.1))
    plt.ylabel("估算值", fontsize=14)
...
def load() -> Tuple[List[DelayInstance], List[LossInstance]]:
    ...
    #         for l in lines:
    #             content = l.split(" ")
    #             delay_instances.append(DelayInstance(min=float(content[-5]), max=float(
    #                 content[-4]), mean=float(content[-3]), std=float(content[-2]), label=float(delay_label)))
